-- RNN/LSTM training for the two graph sequence tasks.
--
-- Yujia Li, 10/2015
--
...
cmd:option('-nval', 100, 'number of validation instances, this will not be used if datafile.val exists')
...
x_train, t_train, end_symbol = seq_data.load_rnn_data_from_file(opt.datafile)
...
if lfs.attributes(opt.datafile .. '.val') == nil then
    ...
    if uniform_length then
        x_train, t_train, x_val, t_val = babi_data.split_set_tensor(x_train, t_train, x_train:size(1) - opt.nval, opt.nval, true)
    else
        x_train, t_train, x_val, t_val = babi_data.split_set_input_output(x_train, t_train, #x_train - opt.nval, opt.nval, true)
    end
else
    print('Loading validation data from ' .. opt.datafile .. '.val')
    x_val, t_val = seq_data.load_rnn_data_from_file(opt.datafile .. '.val', end_symbol)
end
...
function feval(x)
    ...
    if uniform_length then
        ...
        y = net:forward(x_batch, t_batch:size(2))
        t_batch = torch.reshape(t_batch, t_batch:nElement(), 1)
...
function eval_loss(model, x, t)
    if uniform_length then
        local n_targets = t:size(2)
        t = torch.reshape(t, t:nElement(), 1)
        return c:forward(model:forward(x, n_targets), t)
    else
        local loss = 0
        for i=1,#x do
            local tt = t[i]
            local n_targets = tt:size(2)
            tt = torch.reshape(tt, tt:nElement(), 1)
            loss = loss + c:forward(model:forward(x[i], n_targets), tt)
        end
        return loss / #x
    end
end
...
function eval_err(model, x, t)
    if uniform_length then
        local ntargets = t:size(2)
        local pred = model:predict(x, t:size(2))
        pred = pred:typeAs(t)
        if ntargets > 1 then
        ...
        local err = 0
        for i=1,#x do
            local ntargets = t[i]:nElement()
            local pred = model:predict(x[i], t[i]:nElement())
            if ntargets > 1 then
                err = err + pred:typeAs(t[i]):ne(t[i]):type('torch.DoubleTensor'):sum(2):gt(0):sum()
            else
