...
--
-- This RNN implementation takes a sequence of token IDs as input and makes a 
-- prediction about the sequence at the very end.
--
-- Yujia Li, 10/2015
--

local RNN, BaseRNN = torch.class('rnn.RNN', 'rnn.BaseRNN') 

function RNN:__init(vocab_size, embedding_size, hid_size, output_size, module_dict)
...
function RNN:create_modules()
    rnn.create_or_share('LookupTable', 'input-lookup', self.module_dict, {self.vocab_size, self.embedding_size})
    rnn.create_or_share('Linear', 'ih2h', self.module_dict, {self.embedding_size + self.hid_size, self.hid_size})
...
function RNN:create_fixed_components()
...
function RNN:create_network(n_steps)
    ...
    for i_step=#self.time_slices+1,n_steps do
        ...
        local joint = nn.JoinTable(2)({embedding, h_prev})
        local h_new = nn.Tanh()(rnn.create_or_share('Linear', 'ih2h', self.module_dict, {self.embedding_size + self.hid_size, self.hid_size})(joint))

        self.time_slices[i_step] = nn.gModule({input, h_prev}, {h_new})
    end
end
...
function RNN:forward(input, n_outputs)
    ...
    self.h[1] = self.time_slices[1]:forward({input:select(2,1), self.h_init})
    for i_step=2,self.seq_len do
        self.h[i_step] = self.time_slices[i_step]:forward({input:select(2,i_step), self.h[i_step-1]})
    end
...
function RNN:backward(output_grad)
    if self.n_outputs == 1 then
        ...
        for i_step=self.seq_len,2,-1 do
            h_grad = self.time_slices[i_step]:backward({self.input:select(2,i_step), self.h[i_step-1]}, h_grad)[2]
        end
        self.time_slices[1]:backward({self.input:select(2,1), self.h_init}, h_grad)
    else
        ...
        local h_grad = torch.zeros(self.n_seqs, self.hid_size)
        for i_step=self.seq_len,2,-1 do
...
function RNN:predict(input, n_outputs)
    n_outputs = n_outputs or 1
...
function RNN:print_model()
    print(string.format('RNN: Lookup %d->%d | Input to Hid %d->%d | Hid to Output %d->%d', 
